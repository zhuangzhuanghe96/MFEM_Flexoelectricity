...
%
% This code generates the results (Fig.9(b), Proposed method) discussed in Section 4.5.
% 
...
% --- [核心选项] ---
conf.el_type  = 'Q8';    % 位移场: 'Q8' 或 'Q9'
conf.lam_type = 'P0';    % 乘子场: 'Q4' 或 'P0'
conf.condense = true;    % true: 凝聚 Lambda/Bubble; false: 求解全系统


% --- [新增] 分析类型 ---
conf.analysis_type = 'static'; % 可选 'static' 或 'modal' (特征频率)
...
mat.rho = 2300;               % 密度 (kg/m^3)

%% 2. 前处理 (Pre-Processing)
% ---------------------------------------------------------------------
% [修改] 使用适配器调用您的 import_mesh_mixed_v3
% 确保 import_mesh_mixed_v3.m 在当前文件夹路径下
...
% --- 示例: 提取上表面 (Tag 2) 的电势数据 ---
fprintf('\n[Data] 提取上表面电势数据...\n');
% boundary_data = extract_boundary_data(mesh, results, 2, 'phi');
% figure(10)
% plot(boundary_data.clean_x,boundary_data.clean_values,"ko-")

% 假设您有一个解析解函数 (例如纯弯曲梁的电势)
load("Exact_coefficient.mat");
u_exact = @(x,y) coeff(1)*x + coeff(2)/x + coeff(3)*besseli(0, x/coeff(7)) + coeff(4)*besselk(0, x/coeff(7)); 
du_dx  = @(x,y) coeff(1) - coeff(2)/x.^2 + (coeff(3)/coeff(7)) * besseli(1, x/coeff(7))...
- (coeff(4)/coeff(7)) * besselk(1, x/coeff(7));
