...
%
% This code generates the results (Fig. 7(c), This study,FE) discussed in Section 4.4.
% 
...
% 挠曲电耦合系数 (C/m)
mat.mu_L = 0.15E-9;     % 纵向挠曲电系数 (f11)
mat.mu_T = 100E-9;      % 横向挠曲电系数 (f12)
mat.mu_S = -1.9E-9;     % 剪切挠曲电系数 (f44)
...
% 绘制能带结构图
figure('Color', 'w', 'Name', 'Phononic Band Structure');
% 将横坐标归一化 (k / (pi/a)) 便于展示
...
title('Band Structure (Flexoelectric Elasticity)', 'FontSize', 12);
grid on; box on;

% 导出数据到文本文件
data = [k_normalized', all_freqs'];
save("MFEM_Flexo_DispersionCurve.txt", "data", '-ascii');
