...
%
% This code generates the results (Fig.9(c), Proposed method) discussed in Section 4.5.
% 
...
f = @() myCoreCode(dof, mesh, info,scl,mat, conf);   % 创建指向局部函数的句柄
t = timeit(f);
fprintf("核心代码用时：%f s\n",t);

function myCoreCode(dof, mesh, info,scl,mat, conf)
    %%3. 刚度组装 (核心修改在内部)
    fprintf('[Main] 组装刚度矩阵...\n');
