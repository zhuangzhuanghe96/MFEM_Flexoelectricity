...
%
% This code generates the results (Fig. 7(b), This study,SG) discussed in Section 4.4.
% 
...
% --- [网格输入] ---
conf.filename = 'mesh_wave2.mphtxt';
...
% --- [分析类型] ---
conf.analysis_type = 'modal'; % 设定求解器模式: 模态分析
conf.num_modes     = 4;       % 计算前 N 阶模态
...
% --- [材料物理参数] ---
mat.C=[173,82,0; 82,173,0; 0,0,108]*1E9;     % 弹性常数
mat.kappa   = [13.28,0;0,13.28]*1E-9;       % 介电常数矩阵 (F/m)
mat.l_scale = 1E-9;                         % 应变梯度特征长度 (m)

% 挠曲电耦合系数 (C/m)
mat.mu_L = 0.15E-9*0;     % 纵向挠曲电系数 (f11)
mat.mu_T = 100E-9*0;      % 横向挠曲电系数 (f12)
mat.mu_S = -1.9E-9*0;     % 剪切挠曲电系数 (f44)

mat.rho = 5996;         % 材料密度 (kg/m^3)
...
%% 4. 边界条件定义 (Boundary Conditions)
fprintf('[Main] 设定边界条件...\n');

% (1) Dirichlet 边界 (如有固定约束或接地)
BC_List = {}; 
% 例: BC_List{end+1} = struct('tag', [5 6], 'type', 'phi', 'value', [0,0]);
[fixed_dofs, fixed_vals_amp] = apply_dirichlet_bc(mesh, dof, BC_List, scl);

% (2) Neumann 载荷 (模态分析中通常无外力)
Load_List = {};
F_global = apply_neumann_load(mesh, dof, Load_List);

% (3) Floquet 周期性匹配条件
p_pairs = [];
% 设定主从边界关系 (例如: Tag 1 为左边界, Tag 4 为右边界)
p_pairs(1).master_tag = 1; 
p_pairs(1).slave_tag  = 4; 

%% 5. 波矢扫描与特征值求解 (Wave Vector Sweeping)
free_dofs = setdiff(1:dof.n_total, fixed_dofs);
fprintf('[Main] 启动波矢扫描与模态求解 (DOF: %d)...\n', length(free_dofs));

% 定义波矢扫描路径 (Irreducible Brillouin Zone, 例如沿 G-X 方向)
k_path = [0; pi/10e-9]; 
num_k_points = 20;
k_values = linspace(k_path(1), k_path(2), num_k_points);

% 预分配频率存储矩阵 [Modes x k_points]
all_freqs = zeros(conf.num_modes, num_k_points);

% 开始扫描
for k_idx = 1:num_k_points
    kx = k_values(k_idx);
    k_vec = [kx, 0]; % 当前波矢
    
    % A. 基于当前波矢生成原始 Floquet 变换矩阵
    [T_raw, ~] = apply_floquet_bc(mesh, dof, p_pairs, k_vec);
    
    % B. 合并 Dirichlet 约束 (将固定约束融入广义变换矩阵)
    T_combined = merge_floquet_dirichlet(T_raw, fixed_dofs);
    
    % C. 求解广义 Floquet 特征值问题 
    % (利用 T'KT 和 T'MT 缩减系统，提取频率)
    [V_full, freqs] = solve_eigen_floquet(K_global, M_global, T_combined, conf.num_modes, false);
    
    all_freqs(:, k_idx) = freqs;
    fprintf('   k = %6.2e: Mode 1 = %8.2f Hz\n', kx, freqs(1));
end

%% 6. 结果后处理与可视化 (Post-Processing & Visualization)
fprintf('\n[Post] 绘制色散曲线 (Dispersion Curve)...\n');

% 绘制能带结构图
figure('Color', 'w', 'Name', ' Band Structure');
% 将横坐标归一化 (k / (pi/a)) 
k_normalized = k_values / (pi/10e-9);

plot(k_normalized, all_freqs, 'b-o', 'LineWidth', 1.5, 'MarkerSize', 5, 'MarkerFaceColor', 'w');
xlabel('Normalized Wave Vector k_x / (\pi/a)', 'FontSize', 11); 
ylabel('Frequency (Hz)', 'FontSize', 11);
title('Band Structure (Strain Gradient Elasticity)', 'FontSize', 12);
grid on; box on;

% 导出数据到文本文件
data = [k_normalized', all_freqs'];
save("MFEM_StrainGradient_DispersionCurve.txt", "data", '-ascii');
