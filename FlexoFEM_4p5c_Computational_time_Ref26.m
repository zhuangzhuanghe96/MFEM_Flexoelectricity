...
%
% This code generates the results (Fig.9(c), Ref.[26]) discussed in Section 4.5.
% 
...
f = @() myCoreCode(dof, mesh, scl, mat);   % 创建指向局部函数的句柄
t = timeit(f);
fprintf("核心代码用时：%f s\n",t);
function myCoreCode(dof, mesh, scl, mat)
    %%3. 刚度矩阵组装 (封装调用)
    [K_global, F_global] = assemble_system(mesh, dof, mat);
...
function [K_global, F_global] = assemble_system(mesh, dof, mat)
    ...
    for e = 1:mesh.n_elems
        ...
        for i = 1:3
            for j = 1:3
                ...
                % Psi - Lambda 耦合
                q_lp = (B_lam' * eye(4) * Bpsi) * detJ * w;
                % u - Lambda 耦合 (弱形式约束: Psi = grad(u))
                q_lu = - (B_lam' * eye(4) * Bg) * detJ * w;
...
function scl = compute_scaling_factors(mat)
    scl.S_phi = 1; scl.S_psi = 1; scl.S_lam = 1;
    fprintf('[Scaling] S_phi = %.2e, S_psi = %.2e\n', scl.S_phi, scl.S_psi);
end
