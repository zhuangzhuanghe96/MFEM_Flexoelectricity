...
%
% This code generates the results (Fig. 7(b), Analytical,SG) discussed in Section 4.4.
% 
...
if isfile('MFEM_StrainGradient_DispersionCurve.txt')
    ...
    L1 = plot(femData(:, 1), femData(:, 2:end), '.', 'MarkerSize', 10, 'Color', [0.85 0.85 0.85]); 
    hold on;
end
...
for k = [0.001 0.02:0.02:1]
    ...
    da = Searoots(@(f, kv) compute_M(f, kv, p), 1E8, 1e9/2, 6.2e11, k);
...
title('Strain Gradient Wave Dispersion');
xlabel('Wave number (\pi/(2h))');
ylabel('Frequency (Hz)');
% save('AnalyticSolution_StrainGradient.txt',"data",'-ascii')
...
function detM = compute_M(f, k, p)
    ...
    % Case 5 & 6 公共部分
    term56 = -p.l^2*p.c44*(k3p2 + k1.*k3p.*bj);
    
    M(5, :) = term56.' .* exp_plus;
    M(6, :) = term56.' .* exp_minus;

    % Case 7 & 8 公共部分
    term78 = -p.l^2*p.c13*k1.*k3p - p.l^2*p.c33.*k3p2.*bj;
    
    M(7, :) = term78.' .* exp_plus;
    M(8, :) = term78.' .* exp_minus;
...
function k3p = getK3(omega, k1, p)
    ...
    Coeffs = [
        p.c33*p.c44*p.l^4, ...
        2*p.c33*p.c44*p.l^2 - p.c13^2*k1^2*p.l^4 + p.c11*p.c33*k1^2*p.l^4 - 2*p.c13*p.c44*k1^2*p.l^4 + 2*p.c33*p.c44*k1^2*p.l^4, ...
        p.c33*p.c44 - 2*p.c13^2*k1^2*p.l^2 - 2*p.c13^2*k1^4*p.l^4 + 2*p.c11*p.c33*k1^2*p.l^2 + 2*p.c11*p.c33*k1^4*p.l^4 - 4*p.c13*p.c44*k1^2*p.l^2 + p.c11*p.c44*k1^4*p.l^4 - 4*p.c13*p.c44*k1^4*p.l^4 + 2*p.c33*p.c44*k1^2*p.l^2 + p.c33*p.c44*k1^4*p.l^4 - p.c33*p.l^2*omega^2*p.rho - p.c44*p.l^2*omega^2*p.rho, ...
        p.c11*p.c33*k1^2 - p.c33*omega^2*p.rho - p.c44*omega^2*p.rho - 2*p.c13^2*k1^4*p.l^2 - p.c13^2*k1^6*p.l^4 - p.c13^2*k1^2 - 2*p.c13*p.c44*k1^2 + 2*p.c11*p.c33*k1^4*p.l^2 + p.c11*p.c33*k1^6*p.l^4 + 2*p.c11*p.c44*k1^4*p.l^2 - 4*p.c13*p.c44*k1^4*p.l^2 + 2*p.c11*p.c44*k1^6*p.l^4 - 2*p.c13*p.c44*k1^6*p.l^4 - p.c11*k1^2*p.l^2*omega^2*p.rho - p.c33*k1^2*p.l^2*omega^2*p.rho - 2*p.c44*k1^2*p.l^2*omega^2*p.rho, ...
        omega^4*p.rho^2 + p.c11*p.c44*k1^4 + 2*p.c11*p.c44*k1^6*p.l^2 + p.c11*p.c44*k1^8*p.l^4 - p.c11*k1^2*omega^2*p.rho - p.c44*k1^2*omega^2*p.rho - p.c11*k1^4*p.l^2*omega^2*p.rho - p.c44*k1^4*p.l^2*omega^2*p.rho
    ];
