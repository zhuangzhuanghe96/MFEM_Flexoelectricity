...
%
% This code generates the results (Table 1) discussed in Section 4.2.
% 
...
% --- [网格输入] ---
conf.filename = 'mesh_patchtest.mphtxt'; % 采用补丁测试专用网格
...
% --- [分析类型] ---
conf.analysis_type = 'static'; % 设定求解器模式: 静态分析

% --- [材料物理参数] ---
mat.E       = 3.6e9;     % 杨氏模量 (Pa)
mat.nu      = 0.38;      % 泊松比
mat.kappa   = 3.0724e-11;% 介电常数 (F/m)
mat.l_scale = 2e-6;      % 内部特征长度尺度 (m)

% 挠曲电耦合系数 (C/m) - 当前测试中设为0以解耦或专注于纯力学/梯度力学测试
mat.mu_L = 0e-6;            % 纵向挠曲电系数
mat.mu_T = 0e-6;            % 横向挠曲电系数
mat.mu_S = 0e-6;            % 剪切挠曲电系数

%% 2. 前处理与自由度映射 (Pre-Processing)
% 读取并解析混合网格文件
...
%% 4. 边界条件与外部载荷施加 (Boundary Conditions & Loads)
fprintf('[Main] 应用 Dirichlet 边界条件...\n');
BC_List = {}; 

% --- [补丁测试 (Patch Test) 解析场定义] ---
% a 和 b 分别为 u 和 v 的高阶多项式系数
% (通过修改 a 和 b 的非零项，可测试如 S11=1, S12=2,等基本变形模式)
a=zeros(6,1); b=zeros(4,1);

% a(2) = 1;                                         %S11=1
% b(3) = 1;                                         %S22=1
% a(3) = 1;   b(2) = 1;                             %2S12=2
a(4) = 0.5;                                       %eta111=1
% a(4) = -0.5 * mat.C(1,2)/mat.C(1,1); a(6) = -0.5; %eta221=1
% a(5) = 1; b(4) = 0.5;                             %2eta121=2
% a(5) = 1;                                         %eta112=1
% a(5) = -0.5*mat.C(1,1)/(mat.C(1,2)+mat.C(3,3));
% b(4) = -0.25*mat.C(1,1)/mat.C(3,3);               %eta222=1
% a(4) = -mat.C(3,3)/mat.C(1,1); a(6) = 1;          %2eta122=2


% 依据系数构造连续的位移场 (u, v) 解析函数
u_fuc = @(x,y) a(1) + a(2)*x + a(3)*y + a(4)*x.^2 + a(5)*x.*y + a(6)*y.^2;
v_fuc = @(x,y) b(1) + b(2)*x + b(3)*y + b(4)*x.^2 ...
    - (2*mat.C(1,1)*a(4) + 2*mat.C(3,3)*a(6)) / (mat.C(1,2) + mat.C(3,3)) * x.*y ...
    - ((mat.C(1,2) + mat.C(3,3))*a(5) + 2*mat.C(3,3)*b(4)) / (2*mat.C(1,1)) * y.^2;

% 对位移场求导，得到对应的辅助应变场 (psi) 解析函数
psi11_fuc = @(x,y) a(2) + 2*a(4)*x + a(5)*y;
psi22_fuc = @(x,y) b(3) ...
    - (2*mat.C(1,1)*a(4) + 2*mat.C(3,3)*a(6)) / (mat.C(1,2) + mat.C(3,3)) * x ...
    - ((mat.C(1,2) + mat.C(3,3))*a(5) + 2*mat.C(3,3)*b(4)) / mat.C(1,1) * y;
psi12_fuc = @(x,y) (a(3) + a(5)*x + 2*a(6)*y) + ...
    (b(2) + 2*b(4)*x - (2*mat.C(1,1)*a(4) + 2*mat.C(3,3)*a(6)) / (mat.C(1,2) + mat.C(3,3)) * y);

% 将解析场作为 Dirichlet 强约束施加到边界 (Tag 1, 3, 12, 5) 上
BC_List{end+1} = struct('tag', [1, 3, 12, 5], 'type', 'u',   'value', {{u_fuc, v_fuc}});
BC_List{end+1} = struct('tag', [1, 3, 12, 5], 'type', 'phi', 'value', 0); % 边界电势接地
BC_List{end+1} = struct('tag', [1, 3, 12, 5], 'type', 'psi', 'value', {{psi11_fuc, psi22_fuc, psi12_fuc}}); 
...
%% 6. 可视化与误差分析 (Visualization & Error Analysis)
% 绘制全局场分布云图
visualize_results(mesh, results);

%% 7. 派生物理场计算与提取 (Derived Fields)
% 计算由主自由度 (u, phi, psi) 导出的二阶物理场：应变、电场、应变梯度
derived_fields = compute_derived_fields(mesh, dof, U_sol, conf, info, scl);

disp("提取的场变量 (Strain + Gradient):[S11,S22,2S12,psi111,psi221,psi121,psi112,psi222,psi122]")
disp([derived_fields.strain_u, derived_fields.grad_psi]);
