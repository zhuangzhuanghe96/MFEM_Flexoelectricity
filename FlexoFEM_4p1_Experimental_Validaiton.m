...
%
% This code generates the results discussed in Section 4.1.
% 
% If you have any questions, please feel free to contact me at: hezz@zju.edu.cn

clc; clear; close all;

%% 1. 仿真全局配置 (Configuration)
% --- [网格输入] ---
conf.filename = 'mesh_transient.mphtxt';

% --- [核心单元选项] ---
conf.el_type  = 'Q8';    % 位移场插值单元: 'Q8' 或 'Q9'
conf.lam_type = 'P0';    % 乘子场单元: 'Q4' 或 'P0'
conf.condense = true;    % true: 在单元级别凝聚 Lambda/Bubble; false: 求解全系统矩阵

% --- [分析类型] ---
conf.analysis_type = 'transient'; % 设定求解器模式: 瞬态分析

% --- [载荷时间函数配置] ---
conf.load.type = 'sine'; % 载荷随时间的演化类型: 'step' (阶跃) 或 'sine' (正弦)
conf.load.freq = 5;      % 正弦载荷频率 (Hz)

% --- [阻尼模型配置] ---
% 瑞利阻尼 (Rayleigh Damping): C = alpha * M + beta * K
% 挠曲电材料通常固有阻尼较小，但为保证高频数值稳定，建议赋予微小阻尼
conf.damping.alpha = 0;  % 质量比例阻尼系数 (主要抑制低频响应)
conf.damping.beta  = 0;  % 刚度比例阻尼系数 (主要过滤高频数值噪音)

% --- [时间积分步长与总时长] ---
conf.time.dt        = 0.2 / 40; % 瞬态计算时间步长 (s)
conf.time.T_end     = 0.5;      % 瞬态分析总时长 (s)
conf.time.save_freq = 1;        % 结果保存频率 (每隔 N 步保存一次全场数据，用于动画或后处理)

% --- [Newmark-Beta 算法参数] ---
% 采用带有微小数值耗散的无条件稳定格式
conf.time.gamma = 0.6;          % 默认 0.5 代表无额外算法阻尼，>0.5 引入数值耗散
conf.time.beta  = 0.3025;       % 推荐公式: (gamma + 0.5)^2 / 4 保证无条件稳定

% 打印当前控制台配置信息
fprintf('[Config] Displacement: %s\n', conf.el_type);
fprintf('[Config] Multiplier:   %s\n', conf.lam_type);
fprintf('[Config] Condensation: %s\n', string(conf.condense));
fprintf('[Config] Time Step:    %.4e s\n', conf.time.dt);

%% 2. 材料属性与前处理 (Material & Pre-Processing)
% --- [材料物理参数] ---
mat.E       = 3.6e9;                        % 杨氏模量 (Pa)
mat.nu      = 0.38;                         % 泊松比
mat.kappa   = [3.0724, 0; 0, 3.0724] * 1e-11; % 介电常数矩阵 (F/m)
mat.l_scale = 2e-6;                         % 应变梯度特征长度 (m)

% 挠曲电耦合系数 (C/m)
mat.mu_L = 0;           % 纵向挠曲电系数 (f11)
mat.mu_T = 1.761e-8;    % 横向挠曲电系数 (f12)
mat.mu_S = 0;           % 剪切挠曲电系数 (f44)

mat.rho = 1300;         % 材料密度 (kg/m^3) - 瞬态动力学必需参数
...
scl = compute_scaling_factors(mat); % 计算缩放因子以平衡刚度矩阵条件数
fprintf('[Scaling] Factors: S_phi = %.2e, S_psi = %.2e\n', scl.S_phi, scl.S_psi);

% --- [构建全局自由度映射] ---
[dof, mesh, info] = build_dof_map_flexible(mesh, conf);

%% 3. 系统矩阵组装 (System Assembly)
tic;
fprintf('[Main] 组装系统矩阵 (K, M, C)...\n');

% 1. 组装全局刚度矩阵 (包含电学、力学及挠曲电耦合块)
K_global = assemble_system_sequential(mesh, dof, mat, conf, info, scl); 

% 2. 组装全局一致质量矩阵 (仅针对位移自由度赋予质量)
M_global = assemble_mass_matrix(mesh, dof, mat, conf, info);

% 3. 组装瑞利阻尼矩阵
C_global = conf.damping.alpha * M_global + conf.damping.beta * K_global;
toc;

%% 4. 边界条件与外部载荷 (Boundary Conditions & Loads)
fprintf('[Main] 应用 Dirichlet 边界条件...\n');
BC_List = {}; 

% (1) 基础约束与动态激励
BC_List{end+1} = struct('tag', 1, 'type', 'u', 'value', [0, 0]);           % 底部全固定
BC_List{end+1} = struct('tag', 7, 'type', 'u', 'value', [NaN, -3.3e-3]);   % 顶部施加 Y 方向的动态位移振幅
BC_List{end+1} = struct('tag', [2, 3], 'type', 'phi', 'value', 0);         % 设定零电势参考面 (接地)

% 提取固定自由度及其稳态幅值 (动态求解器中会乘以 time_factor)
[fixed_dofs, fixed_vals_amp] = apply_dirichlet_bc(mesh, dof, BC_List, scl);

fprintf('[Main] 应用 Neumann 载荷...\n');
Load_List = {};
% 将静态 Neumann 载荷映射到全局向量 (如有)
F_global = apply_neumann_load(mesh, dof, Load_List);
toc;

%% 5. 瞬态求解求解 (Transient Solution)
free_dofs = setdiff(1:dof.n_total, fixed_dofs);
fprintf('[Main] 启动 Newmark-Beta 瞬态求解器 (DOF: %d)...\n', length(free_dofs));

% 调用隐式时间积分算法求解系统响应历史
[history, t_vec] = solve_transient_newmark(K_global, M_global, C_global, F_global, fixed_dofs, fixed_vals_amp, dof, conf);
toc;

%% 6. 结果后处理与可视化 (Post-Processing & Visualization)
% --- [绘制位移历史响应] ---
figure(2);
% 提取 Tag 7 边界上节点随时间变化的 V 方向 (y方向) 物理量
[t, ~, ~, raw_data] = extract_history_by_tag(history, mesh, dof, 7, 'v');
plot(t, raw_data(:, 2), 'ko-', 'LineWidth', 1.5);
xlabel('Time (s)');
ylabel('Displacement v (m)');
title('Displacement History at Tag 7');
grid on;

% --- [提取电极电荷 (电位移积分)] ---
electrode_tag = 3; % 定义顶部电极对应的边界 Tag

% 注意：电荷提取必须使用原始 K_global，因为电学部分不含质量和阻尼项，
% 缩放修正已在 extract_charge_history_by_tag 函数内部通过 scl.S_phi 处理。
[t_vec, Q_vec] = extract_charge_history_by_tag(history, mesh, dof, K_global, electrode_tag, scl);

%考虑实验中结构的厚度b=1E-2m
Q_vec=Q_vec*1E-2;

figure(101);
plot(t_vec, Q_vec, 'b-o', 'LineWidth', 1.5);
grid on;
xlabel('Time (s)');   
ylabel('Total Charge Q (C)');
title(sprintf('Electric Displacement Integral on Tag %d', electrode_tag));

% 保存对比数据
Q_data = [t_vec, raw_data(:, 2), Q_vec];
% save("实验对比.txt", "Q_data", '-ascii');

% --- [全场云图可视化] ---
% 选取第 25 个时间步的瞬态切片进行空间可视化
target_step = 25;
if target_step <= length(history)
    U_sol = history(target_step).u_vec;
    results = extract_results_flexible(U_sol, dof, mesh, scl);
    visualize_results(mesh, results);
else
    fprintf('[Warn] 设定的可视化步数 %d 超出总步数 %d。\n', target_step, length(history));
end
